...
void MST(int * MST1, int * MST2 , int id){
    ...
    while (now < nodes){                                         
        ...
        now++;
        // cout << "\rNode " << now << " / " << nodes << " processed in phase 1 by thread " << id << "." << flush;
    }
    ...
    while (now < nodes){            
        ...
        now++;
        // cout << "\rNode " << now << " / " << nodes << " processed in phase 2 by thread " << id << "." << flush;
    }
    ...
    while (now < nodes){            
        ...
        minl[now] = minloc;
        now++;
        // cout << "\rNode " << now << " / " << nodes << " processed in phase 4 by thread " << id << "." << flush;
    }


    // from here these conflicts are supervertises. 
    // apply normal prims from here
    ...
    while (conflict > 1){
        ...
        while (now < leaf_count) {           // get info
            ...
            int minval = minv[leaf];
                ...
                // if (my_grp[leaf] == 43 || my_grp[leaf] == 3592 || my_grp[leaf] == 4216 || my_grp[leaf] == 659 || my_grp[leaf] == 1199 || my_grp[leaf] == 871)
            ...
            while (now < conflicts_now) {  
                ...
                for (int i = 0; i < leaves_total; i++){
                    ...
                    else if (branch_minv[i] == min){
                        if (branch_minl[i] > minloc){
                            minloc = branch_minl[i];
                            // frmo = i;
                        }
                    }
                ...
                now = now + 1;
                // cout << "\rGroup heads processed: " << now << " / " << conflicts_now << flush;
            }
            // return;
            ...
            // cout << "Remaining conflicts: " << conflict << "\n";
            ...
            while (now < leaves_total) {         
                ...
                while (my_grp[minloc_grp] != minloc_grp){ //cout << "my_grp_minloc " << minloc_grp << " " << my_grp[minloc_grp] << "\n";
                ...
                if (minloc_grp == my_grp_leaf){        // meaning that minloc is in same group as leaf
                    ...
                    while (order2[curr] == leaf){
                        ...
                        while (my_grp[curr_minloc_grp] != curr_minloc_grp){// cout << "my_grp_curr_minloc " << curr_minloc_grp << " " << my_grp[curr_minloc_grp] << "\n";
                ...
                now = now + 1;  
            //   cout << "\rThread " << id << " prepared " << now << " / " << leaves_total << " leaves for next iteration." << flush;       
            } 
            if (conflict <= 1) return;
            // cout << "Rem/aining conflicts: " << conflict << "\n";
            // now = now + 1;
            // return;
    }
    return;
